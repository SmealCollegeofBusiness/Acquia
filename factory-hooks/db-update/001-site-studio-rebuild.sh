...
echo "Rebuild Site Studio on $DOMAIN domain in $ENVIRONMENT environment on the $SITEGROUP subscription."

# Rebuild site studio.
DRUSH_PATHS_CACHE_DIRECTORY="$cache_dir" $drush -l $DOMAIN cohesion:rebuild
result=$?
