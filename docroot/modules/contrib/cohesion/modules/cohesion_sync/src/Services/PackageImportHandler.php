...
class PackageImportHandler {
  ...
  protected function createImportBatch(): array {
    ...
    if ($this->createStorageComparer()->createChangelist()->hasChanges()) {
      ...
      // For each config to be deleted, retain everywhere it is used
      // in order to rebuild the in use system. This is to make sure
      // the in use table is up-to-date after delete or replacement
      // with a new UUID.
      if (!empty($this->storageComparer->getInuseDelete())) {
        $batch_builder->addOperation(
          [BatchImportController::class, 'handleInuse'],
          [$this->storageComparer->getInuseDelete()]
        );
      }
