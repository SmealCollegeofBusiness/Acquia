...
/**
 * Switch profile from acquia_cms to minimal.
 */
function acquia_cms_update_8004() {
  switch_profile_to_minimal("minimal");
}
 ...
 *   The name of profile to switch to.
 */
function switch_profile_to_minimal(string $profile_to_install = "minimal") {
  $config_factory = \Drupal::configFactory();
  $profile_to_remove = $config_factory->getEditable('core.extension')->get('profile');
  if ($profile_to_remove == 'acquia_cms') {
    ...
    \Drupal::service('keyvalue')->get('system.schema')->delete($profile_to_remove);
    \Drupal::service('keyvalue')->get('system.schema')->set($profile_to_install, 8000);

    // Clear caches again.
    drupal_flush_all_caches();
    \Drupal::logger('acquia_cms')->notice('Profile switched from acquia_cms to minimal.');
    // Update logo and favicon path.
    $acquia_cms_common_path = \Drupal::moduleHandler()->getModule('acquia_cms_common')->getPath();
    // Update favicon path.
    if ($config_factory->getEditable('system.theme.global')->get('favicon.path') == 'profiles/contrib/acquia_cms/acquia_cms.png') {
      $config_factory->getEditable('system.theme.global')
        ->set('favicon.path', '/' . $acquia_cms_common_path . '/acquia_cms.png')
        ->save();
    }
    // Update logo path.
    if ($config_factory->getEditable('system.theme.global')->get('logo.path') == 'profiles/contrib/acquia_cms/acquia_cms.png') {
      $config_factory->getEditable('system.theme.global')
    ...
    \Drupal::logger('acquia_cms')->notice('You are not running on Acquia CMS profile.');
  }
}
